...
def main():
    ...
    file = ur.open("/home/loch/Summer2022/MLTopoCluster/data/Akt4LCTopo.inclusive_topo_cluster_incomplete.root")
    tree = file["ClusterTree"]
    df = tree.arrays(library="pd")

    df = df[(df["cluster_ENG_CALIB_TOT"] > 0.3) & (df["clusterE"] > 0) & (df["cluster_FIRST_ENG_DENS"] > 0) & (df["cluster_CENTER_LAMBDA"] > 0)]
    ...
    output_path_data = file_path + ".npy"
    ...
    # frac = np.sum((df["r_e_calculated"] > 10).to_numpy()) / (df.shape[0] * 1.0)
    # print(frac)

    # plots before preprocessing
    #make_all_plots(df, output_path_figures_before_preprocessing)
    ...
    # just normalizing
    field_names = ["clusterEtaCalib", "cluster_CENTER_MAG", "nPrimVtx", "avgMu"]
    for field_name in field_names:
        x = df[field_name]
        x, mean, std = normalize(x)
    ...
    if save:
        ...
        np.save("all_info_df_train.npy", train)
        np.save("all_info_df_test.npy", test)
        np.save("all_info_df_val.npy", val)
